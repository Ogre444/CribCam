...
class MainWindow(ctk.CTk):
    ...
    def _scan_loop(self):
        while self._running:
            self._do_capture()
            for _ in range(int(self._cfg.interval * 10)):
    ...
    def _do_capture(self, force: bool = False):
        self.after(0, lambda: self._status_var.set(t("status_analyzing")))
        frame = self._camera.capture()
        if frame is None:
        ...
        if result.cached:
            ...
            self.after(0, lambda s=status: self._status_var.set(s))
            return
    ...
    def _on_trying(self, backend_name: str):
        self.after(0, lambda n=backend_name: self._status_var.set(f"⏳ {n}..."))
